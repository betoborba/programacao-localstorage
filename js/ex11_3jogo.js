const frm = document.querySelector("form"); // cria a referência aos elementos do html 
...
let palavraSorteada; // Declara as variáveis globais
let dicaSorteada;
...
window.addEventListener("load", () => {
    ...
    if (!localStorage.getItem("jogoPalavra")) {
        alert("Cadastre palavras para jograr"); // exibe o alerta
        frm.inLetra.disabled = true; // Disabilita inLetra e botões
        frm.btJogar.disabled = true; 
        frm.btVerdica.disabled = true;
    }

    // obtém conteúdo do localStorage e serpa em elementos de vetor
...
frm.btVerdica.addEventListener("click", ()=> {
    // verifica se o jogador ja clicou anteriormente no botão
    ...
    trocarStatus(chances) // trocar a imagem
